...
class DefaultCoordinator:
    ...
    def update_status(self):
        if self.status:
            return self.status.format(done=len(self.results), total=self.total, active=self.active, n=self.n)
        return None
...
class Task:
    def __init__(self, worker, coordinator=None, profile_path=''):
        self.worker = worker
        self.coordinator = coordinator if coordinator is not None else DefaultCoordinator()
        self.profile_path = profile_path
        self._process = None
    ...
    def run(self, tasks, num_workers=None):
        ...
        return process.run(tasks, is_async=False)
