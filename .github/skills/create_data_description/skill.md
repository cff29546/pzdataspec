...
- Place final `.ksy` specs under `data_spec/spec/<world_version>/` to match the existing directory hierarchy.
...
### Game Logic
- Decompiled game code is in `output/decompiled/<game_version>_<world_version>/`. It's not version-controlled but can be generated by running `scripts\decompile_game.bat` from the repo root.
...
- Common objects and entry points:
  - `IsoChunk` represents a single binary file from save data (typically a 10x10 or 8x8 tile area). Its serialization logic is in `zombie.iso.IsoChunk.LoadFromDisk` and `zombie.iso.IsoChunk.Save` methods, found in `output/decompiled/<game_version>_<world_version>/zombie/iso/IsoChunk.java`. This is the primary parser target for the project, and the most complex one.
